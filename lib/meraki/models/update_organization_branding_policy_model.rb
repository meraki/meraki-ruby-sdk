...
module Meraki
  # UpdateOrganizationBrandingPolicyModel Model.
  class UpdateOrganizationBrandingPolicyModel < BaseModel
    ...
    def self.from_hash(hash)
      ...
      admin_settings = AdminSettingsModel.from_hash(hash['adminSettings']) if
        hash['adminSettings']
      help_settings = HelpSettings1Model.from_hash(hash['helpSettings']) if
        hash['helpSettings']

      # Create object from extracted values.
      UpdateOrganizationBrandingPolicyModel.new(name,
                                                enabled,
                                                admin_settings,
                                                help_settings)
    end
