...
module Meraki
  ...
  class WebhookLogsController < BaseController
    @instance = WebhookLogsController.new
    ...
    # the data. The maximum lookback period is 90 days from today.
    ...
    # client applications. The link for the first, last, prev, or next page in
    # the HTTP Link header should define it.
    # @param [String] url Optional parameter: The URL the webhook was sent to
    # @return Mixed response from the API call
    def get_organization_webhook_logs(options = {})
      # Validate required parameters.
      validate_parameters(
        'organization_id' => options['organization_id']
      )
      # Prepare query url.
      _path_url = '/organizations/{organizationId}/webhookLogs'
